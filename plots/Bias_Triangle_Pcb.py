...
g.triangle_plot([mcmc1.samples,mcmc2.samples],filled=[False,False],params=plot_pars,legend_labels=['Data $P_\mathrm{cc}$ model $P_\mathrm{mm}$','Data $P_\mathrm{cc}$ odel $P_\mathrm{cc}$'],contour_lws=[1,1],colors=colors,    contour_colors=colors)
g.legend.set_title(r'Photometric optimistic')
plt.style.use('../../Euclid_KP_nu/plots/plot-style-triangle.txt')

tick_array= [[0.7, 0.8], [0.313, 0.318], [0.806, 0.814], [0.1, 0.25], [1.31, 1.33], [1.49,1.51]]
for i in range(len(cosmo_pars)):
...
g.export('./bias_plots/P_MN_Pcb_v_Pmm_bias.pdf')
